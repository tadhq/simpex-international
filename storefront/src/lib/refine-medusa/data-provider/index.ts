...
// TODO migrate to qs
import { axiosInstance, generateFilter } from '@/lib/refine-medusa/utils';
...
const DataProvider = (
  ...
  return {
    getList: async ({ resource, pagination, filters }) => {
      ...
      const hasQuerySep = url.includes('?');
      ...
      const { data } = await httpClient.get(
        `${url}${hasQuerySep ? '' : '?'}${queryString.stringify(query, stringifyConfig)}&${queryString.stringify(queryFilters, stringifyConfig)}`
      );

      let resourceName = resource;
      if (hasQuerySep) {
        const resourceSegments = resource.split('?');
        resourceName = resourceSegments[0];
      }

      return {
        data: data[resourceName.replace(/-/g, '_')],
        total: data.count,
      };
    },
    ...
    updateMany: async ({ resource, ids, variables }) => {
      const response = await Promise.all(
        ids.map(async (id) => {
          const { data } = await httpClient.post(`${apiUrl}/${resource}/${id}`, variables);
          return data;
        })
      );
    ...
    deleteMany: async ({ resource, ids, variables }) => {
      const response = await Promise.all(
        ids.map(async (id) => {
          const { data } = await httpClient.delete(`${apiUrl}/${resource}/${id}`, variables || {});
          return data;
        })
      );
      return { data: response };
